...
if(alive && o_GUI_single.game_active)
{
var x_input = (keyboard_check(ord("D"))-keyboard_check(ord("A")))
var y_input = (keyboard_check(ord("S"))-keyboard_check(ord("W")))


var shooty = keyboard_check(vk_down)-keyboard_check(vk_up)
var shootx = keyboard_check(vk_right)-keyboard_check(vk_left)
...
//check below and see what color we on
...
if((shooty !=0 || shootx!=0) && shootingtimer ==0)
{
	...
	if(shooty!=0)
	{
	with(instance_create_layer(x,y+shooty*player_size[1]/2,"Bullets",o_bullet_sp))
	{
		speed = 25;
	...
	{
		with(instance_create_layer(x+shootx*player_size[1]/2,y,"Bullets",o_bullet_sp))
		{
			speed = 25;
...
}else if(!alive && o_GUI_single.game_active)
{
